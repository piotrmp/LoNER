...
# Define the model
bert_body = BertPTC_sl(float_type=tf.float32, num_labels=29, max_seq_length=sequence_length,
...
def train_step(input_ids, input_mask, type_ids, label_vec, label_mask):
    with tf.GradientTape() as tape:
        ...
        numpy_mask = np.logical_or(important_labels_argmax != 28,
                                   important_logits_argmax != 28)  # None label is the last one
...
for epoch in epoch_bar:
    ...
    train_semeval_preds = BIOPreds2semeval(arrayed_logits_list=train_predictions,
    ...
    dev_semeval_preds = BIOPreds2semeval(arrayed_logits_list=dev_predictions,
    ...
    test_semeval_preds = BIOPreds2semeval(arrayed_logits_list=test_predictions,
