...
params={'residual_connection':bool(variant & (1<<0)),
        ...
        'with_mu':bool(variant & (1<<3)),
        'reversed':bool(variant & (1<<4)),
        'dropout':bool(variant & (1<<5)),
        'score_input' : False,
        'random_init' : False,
        'presoftmax' : False,
        'bert_input' : True}
...
bert_body = Models.adaptive_conv.BertPTC_slaner(float_type=tf.float32, num_labels=len(k_lengths),
                             max_seq_length=sequence_length, hub_path=bert_model_path, kernels=k_lengths, arch_params=params)

SL_Bert = bert_body.getSimpleModel()
SL_Bert.summary()
...
def train_step(input_ids, input_mask, type_ids, label_vec, label_mask):
    with tf.GradientTape() as tape:
        ...
        numpy_mask = np.logical_or(important_labels_argmax != len(k_lengths),
                                   important_logits_argmax != len(k_lengths))  # None label is the last one
