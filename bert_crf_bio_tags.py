from data_processing.into_bert_2 import prepare_single_label_BIO_input_arrays, createZipDataset
from data_processing.into_bert import BIO_dict, PTC_categories_dict
from data_processing.processing import output_artIDs_tokens_offsets
from data_processing.converting_results import CRF_BIO_preds2semeval
...
train_input_ids, train_input_masks, train_input_type, train_labels, train_booleans = prepare_single_label_BIO_input_arrays(
...
dev_pred_meta = output_artIDs_tokens_offsets(texts_path=dev_articles_path, vocab_path=vocabulary, nested=False,
                                             only_double_enter=False, sentence_length=sequence_length)
dev_input_ids, dev_input_masks, dev_input_type, dev_labels, dev_booleans = prepare_single_label_BIO_input_arrays(
...
test_input_ids, test_input_masks, test_input_type, test_labels, test_booleans = prepare_single_label_BIO_input_arrays(
...
# Define the model
bert_body = BertPTC_CRF(float_type=tf.float32, num_labels=29, max_seq_length=sequence_length,
...
def train_step(input_ids, input_mask, type_ids, label_vec, label_mask):
    with tf.GradientTape() as tape:
        ...
        numpy_mask = np.logical_or(important_labels_argmax != 28,
                                   important_sequence != 28)  # None label is the last one
...
def validation_step(input_ids, input_mask, type_ids, label_vec, label_mask):
    ...
    try:
        ...
        loss = tf.reduce_mean(-crf_log_likelihood(potentials, labels_vec_argmax, sequence_length, chain_kernel)[0])
    important_labels_argmax = tf.boolean_mask(labels_vec_argmax, label_mask)
    numpy_mask = np.logical_or(important_labels_argmax != 28, important_sequence != 28)  # None label is the last one
    masked_labels_np = tf.boolean_mask(important_labels_argmax, numpy_mask)
    masked_sequence = tf.boolean_mask(important_sequence, numpy_mask)
    if np.logical_and(np.any(masked_labels_np), np.any(masked_sequence)):
        batch_accuracy = accuracy_score(masked_labels_np, masked_sequence)
...
for epoch in epoch_bar:
    ...
    train_semeval_preds = CRF_BIO_preds2semeval(decoded_sequences_list=train_predictions,
    ...
    dev_semeval_preds = CRF_BIO_preds2semeval(decoded_sequences_list=dev_predictions,
    ...
    test_semeval_preds = CRF_BIO_preds2semeval(decoded_sequences_list=test_predictions,
